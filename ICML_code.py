...
while count<len(Deltavec):
 ...
 positivepart=lambda y: ((k/n)*(1/(Sigma*np.sqrt(2*math.pi)))*np.exp(-y**2/(2*Sigma**2))                          + (1-k/n)*(1/(2*Sigma*np.sqrt(2*math.pi)))                          *(np.exp(-(y-1)**2/(2*Sigma**2))                          +np.exp(-(y+1)**2/(2*Sigma**2))))                          *(np.log(1/(2*Sigma*np.sqrt(2*math.pi)))-(1/(2*Sigma**2))*(y+1)**2                           + 2*y/Sigma**2 + np.log((2*k/n)*np.exp((1-2*y)/(2*Sigma**2))                          +(1-k/n)*(1+np.exp(-2*y/Sigma**2))))
 iden1,err1=integrate.quad(positivepart, 0, 400)
 negativepart=lambda y: ((k/n)*(1/(Sigma*np.sqrt(2*math.pi)))*np.exp(-y**2/(2*Sigma**2))                          + (1-k/n)*(1/(2*Sigma*np.sqrt(2*math.pi)))                          *(np.exp(-(y-1)**2/(2*Sigma**2))                          +np.exp(-(y+1)**2/(2*Sigma**2))))                          *(np.log(1/(2*Sigma*np.sqrt(2*math.pi)))-(1/(2*Sigma**2))*(y-1)**2                           -2*y/Sigma**2+ np.log((2*k/n)*np.exp((1+2*y)/(2*Sigma**2))                          +(1-k/n)*(1+np.exp(2*y/Sigma**2))))
...
def etafunanderi(x,k,n,tau,Delta):
 ...
 y=((1/2)*(1-k/n)*(np.exp(x/tau**2)-np.exp(-x/tau**2)))/((k/n)*np.exp(1/(2*tau**2))                            +(1/2)*(1-k/n)*(np.exp(x/tau**2)+np.exp(-x/tau**2)))
