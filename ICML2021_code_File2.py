...
for count in range(len(betavec)):
 ...
 MSEfundouter=np.zeros(maxouter)
 out=0 
 while out<maxouter:
 # Generate S,A,y
  s=np.zeros(n)
  ...
  iter=0
  pderi=0
  tau=np.sqrt(Delta*n**(1-beta)+Delta/alpha)
  while iter<iterAMP:
    ...
    G=np.mean(v)
    tau=np.sqrt(Delta*n**(1-beta)+(n**(1-beta)/alpha)*G)
    iter+=1
  sumup=0
...
ax.set_xlabel(r'$\beta$') #corresponding to alpha in the manuscript
ax.set_ylabel('MSE')
plt.legend(('Fundamental Limit','AMP (10 iterations)'))
ax.set_xlim((0, 1))
ax.set_ylim((0, 0.5))
ax.grid(True)
plt.savefig("ICML2021sup.pdf")
